...
class OpKineticNetworkStressAllPurification: OpKineticBasePurification() {
    override fun run(targetPos: BlockPos, env: CastingEnvironment): List<Iota> {
        ...
        return listOf( DoubleIota(kn.calculateCapacity() * 1.0))
    }

    companion object{
        const val NAME = "kinetic_network_stress_all_purification"
        const val STROKE = "qadedaqe"
